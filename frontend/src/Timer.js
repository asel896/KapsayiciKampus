...
const Timer = ({ activeTodo }) => {
  ...
  const handleAuth = async () => {
    ...
    try {
        console.log("İstek gönderiliyor kapı:", endpoint);
        ...
        if (response.ok) {
            if (authMode === 'signup') {
                alert("🎉 KAYIT BAŞARILI! Şimdi bu bilgilerle giriş yapabilirsiniz.");
                setAuthMode('login'); 
                ...
                setUser({ 
                    ...
                    name: data.user.firstName || data.user.first_name || 'Kullanıcı'
                });
                setIsSettingsOpen(false); 
            }
        } else {
            alert("Sunucu Hatası: " + (data.error || "İşlem başarısız"));
        }
    } catch (err) {
        console.error("Bağlantı hatası:", err);
        alert("SUNUCUYA BAĞLANILAMADI! Lütfen terminalde 'node index.js' yazan pencerenin açık ve hatasız olduğundan emin ol.");
    }
};
